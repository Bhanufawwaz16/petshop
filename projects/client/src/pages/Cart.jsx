...
export default function Cart() {
  ...
  const generateCart = async () => {
    const cartResponse = await api
      .get
      //   "/cart/" + user.id + `/${branchesGlobal.selectedBranch.id}`
      ();
      ...
      {cart.length < 1 ? (
        <section>
          <div className="text-3xl flex flex-row justify-center items-center font-bold text-slate-400">
            <img className="w-1/4 mr-10" src={NoAddress} alt="No Address" />
                      ...
                      <div className="ml-4 flex flex-1 flex-col justify-between sm:ml-6">
                        <div className="relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0">
                          <div>
                            ...
                            <p className="mt-1 text-sm font-medium text-gray-900">
                              {numToIDRCurrency(value.m_product.price)}
                            </p>
                          </div>
                          ...
                          <div className="flex flex-row mt-4 sm:mt-0 sm:pr-9">
                            <div>
                              <button
                                className="mx-3"
                                type="button"
                                onClick={() => deleteOne(value.m_product_id)}
                              >
                            ...
                            <div>
                              <button
                                ...
                                className="mx-3"
                                onClick={() =>
                                  addOne(value.m_product_id, value.m_user_id)
                                }
                              >
                        ...
                        {/* {value.m_product.Vouchers[0] ? (
                          <div className="bg-red-200 text-red-900 rounded-full font-semibold items-center flex justify-center w-1/3 p-1 text-xs">
                            {value.Product.Vouchers[0].voucher_type ===
                            "Buy One Get One"
                              ? value.m_product.Vouchers[0].voucher_type
                              : "Discount " +
                                numToIDRCurrency(
                                  value.m_product.Vouchers[0].amount
                                )}
                          </div>
                        ) : (
                          <></>
