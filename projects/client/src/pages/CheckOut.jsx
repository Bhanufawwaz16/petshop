...
export default function CheckOut() {
  ...
  useEffect(() => {
    if (cart.length > 0) {
      //   setOrigin(cart[0].m_product.Stocks[0].Branch.kota)
    }
  }, [cart]);

  useEffect(() => {
    if (address) {
      const findDestination = cityRaja.find(
        (val) => val.city_name === address.kota
      );
      if (findDestination) {
  ...
  const calculateTotalPrice = () => {
    let totalPrice = 0;
    cart.forEach((value) => {
      totalPrice += value.m_product.price * value.qty;
    });
    return totalPrice;
  };
  ...
  const voucherTotalBelanja = voucher.filter(
    (value) => value.voucher_type === "Total Belanja"
  );
      ...
      if (value.Product.Vouchers && value.Product.Vouchers.length > 0) {
        ...
        if (voucher.amount) {
          ...
          const discountAmount =
            value.Product.price * value.qty * voucher.percentage;
      ...
      <div className="mx-auto max-w-2xl px-4 pt-16 pb-24 sm:px-6 lg:max-w-7xl lg:px-8">
        ...
        >
          <section aria-labelledby="cart-heading" className="lg:col-span-7">
              ...
              <div className="py-3">
                <div className="grid grid-cols-2 w-full">
                  ...
                  <div className="flex justify-end items-center">
                    <p className="text-center font-medium text-slate-400 mr-2">
                      Main Address
                    </p>
                  </div>
                </div>
            ...
            <ul
              role="list"
              className="divide-y divide-gray-200 border-t border-b border-gray-200 mt-3"
            >
              {cart.map((value) => (
                <li key={value.id} className="flex py-6 bg-white sm:py-10">
                  <div className="flex-shrink-0">
                    <img
                      src={`${process.env.REACT_APP_PRODUCT_IMG_BASE_URL}/${value.m_product.image_url}`}
                      alt={`${value.m_product.name} image`}
                  ...
                  <div className="ml-4 flex flex-1 flex-col justify-between sm:ml-6">
                    <div className="relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0">
                      <div>
                        ...
                        <p className="mt-1 text-sm font-medium text-gray-900">
                          {numToIDRCurrency(value.m_product.price * value.qty)}
                        </p>
                      </div>
                    ...
                    {/* <p>From branch: {value.m_product.Stocks[0].Branch.kota}</p> */}
                  </div>
                </li>
              ))}
            </ul>
              ...
              <dl className="mt-6 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <dd className="text-sm font-medium text-gray-900" id="jne">
                    ...
                    >
                      ...
                      {ongkir.map((value) => (
                        <option key={value.service} value={value.cost}>
                          {value.service} | {numToIDRCurrency(value.cost)} est.{" "}
                          {value.days}
                        </option>
                      ))}
                    </select>
                    ...
                    <select className="block w-full appearance-none rounded-md border border-gray-300 pl-3 pr-8 py-2 placeholder-gray-400 shadow-sm focus:border-orange-500 focus:outline-none focus:ring-orange-500 sm:text-sm">
                      {voucherTotalBelanja.map((value) => {
                        if (value.min_purchase <= totalPriceWithShipping) {
                          return (
                            <option key={value.id} value={value.amount}>
                              - {numToIDRCurrency(value.amount)}
                            </option>
                          );
                        }
                        return (
                ...
                <div className="flex items-center justify-between border-t border-gray-200 pt-4">
                  ...
                  <dd className="text-base font-medium text-gray-900">
                    {numToIDRCurrency(totalPriceWithShipping)}
                  </dd>
                </div>
              </dl>
